...
class UploadRawVideo implements ShouldQueue
{
    ...
    {
        $rawVideoFile = new File($this->tempVideoPath);
        $rawVideoFilePath = Storage::disk('s3')->putFileAs($this->videoUrlSavePath, $rawVideoFile, 'raw.mp4');
        //This is to give the illusion that all the resolutions are uploaded while the processing happens later.
        Storage::disk('s3')->delete([$this->videoUrlSavePath . '/360.mp4', $this->videoUrlSavePath . '/480.mp4', $this->videoUrlSavePath . '/720.mp4', $this->videoUrlSavePath . '/1080.mp4']);
        Storage::disk('s3')->copy($rawVideoFilePath, $this->videoUrlSavePath . '/360.mp4');
        Storage::disk('s3')->copy($rawVideoFilePath, $this->videoUrlSavePath . '/480.mp4');
        Storage::disk('s3')->copy($rawVideoFilePath, $this->videoUrlSavePath . '/720.mp4');
